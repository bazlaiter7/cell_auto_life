...
public:
	...
	void CalculateNextFrame(std::string* console_ptr, int*); ///Calculates the next frame given the current frame
	void ChangedWindowSize(int, int); ///Changes the size of the gameboard to fit to the new resolution
	void SetBoardPosition(int, int, bool, std::string*, int*, AudioDevicePtr device); ///Sets the board value at a given index
	void DrawGrid(); ///Draws a grid onto the window depending on the boards width and height
	void DrawBoard(std::string* console_ptr, int*); ///Draws the cells of the board onto the main screen
	void DrawMouseHover(double, double); ///Draws the square over the cell that the player is hovering over
	void MouseClick(double, double, bool, int*, std::string*, AudioDevicePtr device); ///Enables/disables the cell the player clicked on
	void ResetBoard(std::string* console_ptr, int*); ///Resets all the cells on the board to false
	void LoadPreconfiguration(int, std::string* console_ptr, int*, AudioDevicePtr device); ///Loads a premade sequence
};